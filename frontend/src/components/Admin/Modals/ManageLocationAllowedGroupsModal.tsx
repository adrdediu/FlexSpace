...
const useStyles = makeStyles({
  ...
  groupItemSelected: {
    ...
    borderLeft: tokens.colorBrandStroke1,
...
interface ManageLocationAllowedGroupsModalProps {
  open: boolean;
  onClose: () => void;
  location: Location | null;
  onSetGroups: (locationId: number, groupIds: number[]) => Promise<void>;
}

export const ManageLocationAllowedGroupsModal: React.FC<ManageLocationAllowedGroupsModalProps> = ({
  open,
  onClose,
  location,
  onSetGroups,
  ...
  useEffect(() => {
    ...
    setSelectedGroupIds(new Set((location.allowed_groups ?? []).map(g => g.id)));
    setSearchQuery('');
    fetchGroups();
  }, [open, location?.id]);
  ...
  const fetchGroups = async () => {
    ...
    try {
      const groups = await groupApi.getGroupsByLocation(location.id);
  ...
  const handleSelectAll = () => setSelectedGroupIds(new Set(filtered.map(g => g.id)));
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await onSetGroups(location.id, Array.from(selectedGroupIds));
      onClose();
    } catch (err: any) {
    ...
    >
      <div className={styles.content}>
        ...
        <div className={`${styles.accessBanner} ${isOpen ? styles.accessOpen : styles.accessRestricted}`}>
          {isOpen
            ? <><LockOpen20Regular /><span>Open access — all users can access this location</span></>
            : <><LockClosed20Regular /><span>Restricted — only {selectedCount} {selectedCount === 1 ? 'group' : 'groups'} can access this location</span></>
          }
        </div>
        ...
        <div className={styles.infoBar}>
          ...
          <Text size={200} style={{ color: tokens.colorNeutralForeground2 }}>
            Select the groups that can access this location and book its rooms.
            Unchecking all groups opens the location to everyone.
            Room-level groups further restrict which rooms each group can book.
          </Text>
        </div>
          ...
          <>
            ...
            <div>
              <div className={styles.sectionLabel}>
                ...
                {searchQuery ? ` matching "${searchQuery}"` : ` in ${location?.name ?? 'this location'}`}
              </div>
